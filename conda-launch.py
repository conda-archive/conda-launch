...
def get_input_file():
	...
	p.add_argument('filename', \
				help="Must specify an ipython notebook file with extension .ipynb or a tarball containing a notebook file of the same name")
	p.add_argument('--envname','-e',nargs='?',dest='setenv', \
				help="Optional flag to name the conda environment to be used; if an environment with that name already exists and matches required dependencies it will be used.")
	args = p.parse_args()
...
def create_conda_env(set_envname, depends, create_env=True, env_list=None):
	...
	# try to find a suitable existing env
		...
		if set_envname in env_list:
			...
			try:
				env_output = issue_cmd(["list","-n",conda_params['envname']],conda_api._call_conda)[0].split('\n')[2:-1]
			...
			if missing:
				log_output( "Existing environment named",set_envname,"lacks the following required packages",missing)
				log_output( "Creating a new env instead.",set_envname )
				return create_conda_env(set_envname, depends, True, env_list)
			else:
				log_output( "Existing environment name",set_envname,"satisfies requirements and will be used.")
				return set_envname
